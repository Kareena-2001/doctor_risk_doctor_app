...
class _ReferDoctorFormState extends ConsumerState<ReferDoctorForm> {
  ...
  Future<void> _submit() async {
    final authState = ref.read(authenticationViewModelProvider).valueOrNull ??
        const AuthenticationState();

    if (_firstNameController.text.trim().isEmpty ||
        _lastNameController.text.trim().isEmpty ||
        _mobileController.text.trim().isEmpty ||
        authState.selectedCategory == null ||
        authState.selectedSpeciality == null ||
        authState.selectedDegrees.isEmpty) {
    ...
    await ref
      ...
      categoryId: authState.selectedCategory!.id,
      specialityId: authState.selectedSpeciality!.id,
      degree: authState.selectedDegrees.map((d) => d.name).join(', '),
      remark: _remarkController.text.trim(),
    );
  }

  Future<void> _openDegreePicker(AuthenticationState state) async {
    final notifier = ref.read(authenticationViewModelProvider.notifier);
  ...
  Widget _buildDegreeField(AuthenticationState state) {
  ...
  Widget build(BuildContext context) {
    final authNotifier = ref.read(authenticationViewModelProvider.notifier);
    final authAsync = ref.watch(authenticationViewModelProvider);
    final authState = authAsync.valueOrNull ?? const AuthenticationState();
      ...
      child: Column(
        ...
        children: [
          ...
          _buildReferenceDropdown(
            ...
            isLoading: authState.isCategoryLoading,
            error: authState.categoryError,
            items: authState.categories,
            selected: authState.selectedCategory,
            onRetry: authNotifier.categoryList,
            onSelected: authNotifier.selectCategory,
          ),
          height(Responsive.h(14)),
          _buildReferenceDropdown(
            label: 'Speciality',
            emptyHint: authState.selectedCategory == null
                ? 'Select a category first'
                : 'Select Speciality',
            disabled: authState.selectedCategory == null,
            isLoading: authState.isSpecialityLoading,
            error: authState.specialityError,
            items: authState.specialities,
            selected: authState.selectedSpeciality,
            onRetry: () {
              if (authState.selectedCategory != null) {
                authNotifier.specialityList(
                  categoryId: authState.selectedCategory!.id.toString(),
                );
              }
            },
            onSelected: authNotifier.selectSpeciality,
          ),
          height(Responsive.h(14)),
          _buildDegreeField(authState),
          height(Responsive.h(14)),
          CustomTextField(
            label: 'Remark',
